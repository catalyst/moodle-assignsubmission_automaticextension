...
function xmldb_assignsubmission_automaticextension_upgrade($oldversion) {
    ...
    if ($oldversion < 2025072900) {
        ...
        // If we're upgrading from course limit feature branch rename the table, otherwise create.
        $shorttable = new xmldb_table('assign_automaticextension');
        if ($dbman->table_exists($shorttable)) {
            // Launch rename table for assignsubmission_automaticextension.
            $dbman->rename_table($shorttable, 'assignsubmission_automaticextension');
        } else {
            // Define table assignsubmission_automaticextension to be created.
            $table = new xmldb_table('assignsubmission_automaticextension');
        ...
        upgrade_plugin_savepoint(true, 2025072900, 'assignsubmission', 'automaticextension');
    }
